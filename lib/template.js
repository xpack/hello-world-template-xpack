...
class XpmInitTemplate {
  ...
  async generate (liquidMap, isInteractive) {
    ...
    await makeDir('include')
    await cpFile(path.resolve(templatesPath, 'include/hello-world.h'),
      'include/hello-world.h')
    log.info('File \'include/hello-world.h\' copied.')

    await makeDir('src')
    await cpFile(
      path.resolve(templatesPath, `src/hello-world.${fileExtension}`),
    ...
    await cpFile(path.resolve(templatesPath, '.vscode/tasks.json'),
      '.vscode/tasks.json')
